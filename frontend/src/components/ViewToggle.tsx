...
export default function ViewToggle({ mode, onChange }: { mode: TaskViewMode, onChange: (m: TaskViewMode)=>void }) {
  return (
    <div className="inline-flex border border-[var(--stroke)] rounded-sm overflow-hidden">
      <button
        className={`px-2 py-1 text-sm ${mode==='list' ? 'bg-[#2B2B31]' : 'bg-[#1F1F23] hover:bg-[#25252A]'} border-r border-[var(--stroke)]`}
        aria-pressed={mode==='list'}
        title="List view"
        onClick={()=>onChange('list')}
      >List</button>
      <button
        className={`px-2 py-1 text-sm ${mode==='board' ? 'bg-[#2B2B31]' : 'bg-[#1F1F23] hover:bg-[#25252A]'}`}
